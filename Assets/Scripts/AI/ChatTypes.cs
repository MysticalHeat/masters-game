...
namespace MastersGame.AI
{
    ...
    public static class NpcConversationSupport
    {
        ...
        public static string BuildSystemPrompt(ChatRequest request, bool forceRussianResponses)
        {
            ...
            builder.AppendLine("6. Если игрок тяжело ранен поздно ночью, можешь резко и коротко указать на это.");
            builder.AppendLine("7. Если не знаешь ответ, скажи об этом просто и в рамках мира. Не выдумывай мета-объяснений.");
            builder.AppendLine("8. Отвечай кратко, естественно и по делу, обычно 1-3 короткими фразами.");
            builder.AppendLine(forceRussianResponses
                ? "9. Отвечай только на русском языке."
                : "9. Отвечай на языке игрока, но не выходи из роли.");
            builder.AppendLine("10. Верни только реплику персонажа без служебных пометок и пояснений.");
